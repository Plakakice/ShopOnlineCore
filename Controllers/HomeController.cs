    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult LoadMoreProducts(string category, int page = 1, int pageSize = 8)
        {
            ...
            foreach (var product in products)
            {
                ...
                // Logic price
                string priceHtml = $@"<span class=""item_price"" style=""font-size: 1.3em; font-weight: 700; color: #e74c3c;"">{product.Price.ToString("N0")} ₫</span>";
                if (product.SalePrice.HasValue && product.SalePrice > product.Price)
                {
                    priceHtml += $@"<del style=""color: #999; margin-left: 10px;"">{product.SalePrice.Value.ToString("N0")} ₫</del>";
                }
